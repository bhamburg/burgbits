...
<template>
  <div v-if="data.fetched" class=" font-mono bg-zinc-100 dark:bg-zinc-700 p-4 mb-4 rounded-lg shadow">
    <ClientOnly v-for="shelf in data.shelves">
      ...
      <div v-show="isTable" class="overflow-x-auto">
        <table class="w-full text-sm mb-10" mt--2>
          <thead>
            <tr class="text-left">
              <th v-if="!shelf.title.toLowerCase().includes('current')" class="w-28 p-2">Date</th>
    ...
    <p v-if="data.fetched" class="mb-2 text-sm text-center text-zinc-500">
